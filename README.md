...
✅ **Star Schema Data Model** — FactSales and FactTraffic with Date, Product, Customer, Region, and Channel dimensions
✅ **Event detection from provided data** — event windows taken from the dataset's built-in moment flags (tournament / holiday / league-launch), not external tournament dates — fully within the contest's no-external-data rule
✅ **Acquisition-cohort analysis** — customers flagged by whether their first purchase fell in an event window, to test event-acquired vs baseline-acquired retention
✅ **Time-intelligence DAX** — year-over-year deltas, in-period vs all-time repeat rates, event-vs-baseline revenue separation, and a run-rate baseline
✅ **Executive-first design** — top-line answer surfaces before detail; drill-down and tooltips sit one layer down
✅ **Accessibility** — alt text, strong contrast, intentional reading order, colour never load-bearing
...
- **Data Storytelling:** structuring a report around a single thesis, then honestly revising it when the evidence pushed back
- **Executive Communication:** designing for a skim-first, decision-focused audience and ending every page on a decision
- **Data Modeling:** building a star schema with multiple fact tables and engineered event/cohort flags
- **DAX:** time intelligence, in-period vs all-time metrics, and cohort retention measures
- **Accessibility:** building inclusive, contrast-safe, screen-reader-friendly reports
---
 
## 👤 About Me
 
**Bedoor Ayadh Almareni**
Computer Science Graduate (AI Specialization) | Aspiring Data Analyst
📍 Jeddah, Saudi Arabia
...
**Note:** PitchSide Pro is a fictional company and the dataset is provided for contest/educational purposes. This project is for portfolio and learning purposes.
...
- 🔄 Share it with others learning Power BI
